...
const typeBadgeColor: Record<string, string> = {
  ORDER_NEW: "bg-green-100 text-green-700",
  TRACKING_UPDATED: "bg-blue-100 text-blue-700",
  PRICE_CHANGED: "bg-orange-100 text-orange-700",
  STOCK_LOW: "bg-red-100 text-red-700",
  CLAIM_NEW: "bg-purple-100 text-purple-700",
  SETTLEMENT_READY: "bg-teal-100 text-teal-700",
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        let data = json.data || [];
        if (showUnreadOnly) {
          data = data.filter((n: Notification) => !n.isRead);
        }
              ...
              {notifications.map((n) => (
                <div
                  ...
                  className={`flex items-start gap-3 rounded-lg border p-4 transition-colors ${
                    n.isRead ? "bg-white" : "bg-blue-50 border-blue-200 cursor-pointer"
                  }`}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${typeBadgeColor[n.type] || "bg-gray-100 text-gray-700"}`}>
